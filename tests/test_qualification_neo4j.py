...
def test_qualification_neo4j_integration():
    ...
    try:
        ...
        assert qualification_service.neo4j_service is not None, "Neo4j service should be initialized"
        ...
        try:
            ...
            logger.info(f"✓ Retrieved {len(statuses)} qualification statuses from Neo4j")
            
            # Test getting recommendations
            recommendations = qualification_service.get_program_recommendations_by_qualification(1)
            logger.info(f"✓ Retrieved {len(recommendations)} recommendations from Neo4j")
            
        except Exception as e:
            logger.warning(f"Neo4j data retrieval test failed (expected if no data): {e}")
